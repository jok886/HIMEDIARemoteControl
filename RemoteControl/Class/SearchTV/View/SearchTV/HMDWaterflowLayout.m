...
/** 默认的列数*/
static const NSInteger HMDDefaultColumeCount = 2;
/** 默认的行数*/
static const NSInteger HMDDefaultRowCount = 5;

/** 每一列之间的间距*/
static const NSInteger HMDDefaultColumeMargin = 10;
/** 每一行之间的间距*/
static const CGFloat HMDDefaultRowMargin = 10;
/** 边缘之间的间距*/
static const UIEdgeInsets HMDDefaultEdgeInset = {10, 10, 10, 10};

@interface HMDWaterflowLayout()
...
@implementation HMDWaterflowLayout
...
-(void)prepareLayout {
    ...
    if (self.flowLayoutStyle == HMDVerticalWaterFlow) {
    ...
    }else if (self.flowLayoutStyle == HMDHorizontalWaterFlow){
...
/*
 ...
 {
 if (self.flowLayoutStyle == HMDLineWaterFlow) {
...
- (CGRect)itemFrameOfVHWaterFlow:(NSIndexPath *)indexPath{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(waterFlowLayout:widthForItemAtIndexPath:itemHeight:)]) {
        w = [self.delegate waterFlowLayout:self widthForItemAtIndexPath:indexPath itemHeight:h];
    }
    CGFloat x;
    CGFloat y;
...
- (CGRect)headerViewFrameOfVerticalWaterFlow:(NSIndexPath *)indexPath{
    ...
    if (self.flowLayoutStyle == HMDVerticalWaterFlow) {
        ...
        if (self.rowWidths.count >0) {
            ...
            [self.rowWidths addObject:@(self.collectionView.frame.size.width)];
        }
